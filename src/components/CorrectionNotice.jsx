...
export default function CorrectionNotice({
  ...
  const [reason, setReason] = useState(
    () =>
      localStorage.getItem("corr_reason") ||
      "I corrected all the speakers already and error occured."
  );
  const [output, setOutput] = useState("");
  const [copyBtnText, setCopyBtnText] = useState("Copy Correction Notice");

  useEffect(() => localStorage.setItem("corr_reason", reason), [reason]);

  const handleFileChange = (index, field, value) => {
    const updated = [...correctionFiles];
    updated[index][field] = value;
    setCorrectionFiles(updated);
  };

  const addFileRow = () =>
    setCorrectionFiles([...correctionFiles, { name: "", link: "" }]);

  const removeFileRow = (index) => {
    const updated = correctionFiles.filter((_, i) => i !== index);
    setCorrectionFiles(updated.length > 0 ? updated : [{ name: "", link: "" }]);
  };

  const generateNotice = () => {
    const validFiles = correctionFiles.filter(
    ...
    const noticeText = `CORRECTION NOTICE\nFilename:\n${namesText}\n\nFile Link:\n${linksText}\n\nReason: ${reason}`;
  ...
  const copyNotice = () => {
    ...
    navigator.clipboard
      ...
      .then(() => {
        ...
        setTimeout(() => setCopyBtnText("Copy Correction Notice"), 2000);
      })
  ...
  const clearDraft = () => {
    if (window.confirm("Clear the current draft?")) {
      setCorrectionFiles([{ name: "", link: "" }]);
      setReason("I corrected all the speakers already and error occured.");
      setOutput("");
    }
  };
      ...
      <div className="tat-split-layout">
        <div>
          <div className="header">
            <h2>Error Correction Generator</h2>
          </div>
          <div
